...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("قم بالرد على ملف لضغطه.")
        return
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "مرحبًا ، أنت لست مشرفًا. لا يمكنك استخدام هذا الأمر ، ولكن يمكنك استخدامه في بلدي مساء 🙂"
            )
            return

    mone = await event.reply("⏳️ أرجو الإنتظار...")
...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("الرجاء الرد على ملف zip.")
        return
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
